...
const barber = {
  ...
  reviews: [
    ...
    'Professional and reliable. Has been my airport barber for 2 years.',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', minHeight: 580 }}>
        ...
        <div className="main-content" style={{ overflowY: 'auto' }}>
          ...
          <div className="profile-banner" style={{ margin: '-28px -32px 0', borderRadius: 0 }}>
            <div className="pb-av pb-av-amber">{barber.initials}</div>
          </div>
          <div style={{ paddingTop: 44, marginBottom: 14 }}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 22, fontWeight: 800, marginBottom: 4 }}>{barber.name}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <span className="badge badge-green">&#9733; {barber.rating} &middot; {barber.rides} rides</span>
            </div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
              {barber.airports.map((a) => (
                <span key={a.code} className="badge badge-blue">&#9992; {a.code}</span>
              ))}
              <span className="badge badge-green">&#10003; Safety Protocol</span>
              <span className="badge badge-green">&#10003; Insured &middot; {barber.insuranceProvider}</span>
              {barber.professionalStandard && (
          ...
          <div className="ins-strip" style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--green-pale)', borderRadius: 'var(--r-md)', padding: '7px 11px', marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--green)', fontWeight: 500 }}>{barber.insuranceType} insured &middot; {barber.insuranceProvider} &middot; Active coverage</div>
          </div>

          {/* Vehicle */}
          <div className="t-label" style={{ marginBottom: 8 }}>Vehicle</div>
          <div className="card-surface" style={{ marginBottom: 16 }}>
            <div className="row"><span className="pref-label">Vehicle</span><span className="pref-value">{barber.vehicle.makeModel}</span></div>
            <div className="row"><span className="pref-label">Class</span><span className="pref-value">{barber.vehicle.vehicleClass}</span></div>
            <div className="row"><span className="pref-label">Color</span><span className="pref-value">{barber.vehicle.color}</span></div>
            <div className="row"><span className="pref-label">Passengers</span><span className="pref-value">{barber.vehicle.passengers} max</span></div>
          ...
          <div className="card-surface" style={{ marginBottom: 16 }}>
            <div className="row"><span className="pref-label">Set amount</span><span style={{ fontFamily: "'Syne', sans-serif", fontSize: 14, fontWeight: 800, color: '#3C3489' }}>${barber.rates.setAmount} &middot; agreed per client</span></div>
            <div className="row"><span className="pref-label">Flat local</span><span className="pref-value">${barber.rates.flatLocal} &middot; 10 miles or less</span></div>
            <div className="row"><span className="pref-label">Airport run</span><span className="pref-value">${barber.rates.airportRun} &middot; 50 miles or less</span></div>
            <div className="row"><span className="pref-label">Hourly</span><span className="pref-value">${barber.rates.hourly}/hr</span></div>
            <div className="row"><span className="pref-label">Distance</span><span className="pref-value">${barber.rates.distance} &middot; 51+ miles</span></div>
            <div className="row"><span className="pref-label">Empty return</span><span className="pref-value">${barber.rates.emptyReturn} fixed</span></div>
          </div>

          {/* Availability */}
          <div className="t-label" style={{ marginBottom: 8 }}>Availability</div>
          <div className="card-surface" style={{ marginBottom: 20 }}>
            <div className="row"><span className="pref-label">Days</span><span className="pref-value">{barber.availability.days}</span></div>
            <div className="row"><span className="pref-label">Hours</span><span className="pref-value">{barber.availability.hours}</span></div>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-3)', fontStyle: 'italic', marginBottom: 16, textAlign: 'center' }}>
            The following information is self-reported by the barber and has not been verified by {config.serviceName}.
          </div>
          ...
          <div className="card-surface" style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <span className="badge badge-green">{barber.drivingRecord.yearsLicensed} Years Licensed</span>
              <span className="badge badge-green">10 Year Accident Free</span>
            </div>
            <div style={{ fontSize: 10, color: 'var(--text-3)', fontStyle: 'italic', marginTop: 4 }}>Self-reported by barber</div>
          </div>
        ...
        <div className="right-panel">
          ...
          <button className="btn btn-primary btn-full" onClick={() => router.push(`/join/${barber.airportCode}\u00B7${barber.codeInitials}\u00B7${barber.codeDigits}`)}>
